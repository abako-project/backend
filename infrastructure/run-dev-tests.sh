...
# Verbose level: "info" (default) shows progress and errors, "all" shows debug logs too
VERBOSE_LEVEL="${VERBOSE_LEVEL:-info}"

# Function to output debug messages only if verbose level is "all"
debug_log() {
  if [ "$VERBOSE_LEVEL" = "all" ]; then
...
if [ "$VERBOSE_LEVEL" = "all" ]; then
  echo -e "${BLUE}Verbose level: $VERBOSE_LEVEL (showing all debug logs)${NC}"
fi
echo ""
...
check_service() {
  ...
  while [ $attempt -le $max_attempts ]; do
    ...
    if [ $((attempt % 10)) -eq 0 ]; then
      debug_log "Attempt $attempt/$max_attempts - curl exit code: $CURL_EXIT"
      debug_log "Response: ${RESPONSE:0:100}"
    fi
  ...
  echo -e "${RED}Last error: $RESPONSE${NC}"
  return 1
}
...
check_zombienet_ready() {
  ...
  debug_log "Starting Zombienet verification..."
  ...
  while [ $attempt -le $max_attempts ]; do
    ...
    debug_log "Attempt $attempt - curl response: ${RESPONSE:0:100}..."
    
    FINALIZED_BLOCK=$(echo "$RESPONSE" | grep -o '"result":"[^"]*"' | cut -d'"' -f4)
    
    debug_log "Finalized block hash: $FINALIZED_BLOCK"
    ...
    if [ ! -z "$FINALIZED_BLOCK" ] && [ "$FINALIZED_BLOCK" != "null" ]; then
      ...
      if [ ! -z "$BLOCK_NUMBER" ] && [ "$BLOCK_NUMBER" != "null" ]; then
        ...
        if [ $((attempt % 5)) -eq 0 ]; then
          echo -e "\n${YELLOW}   Current block: $BLOCK_NUM / 25 (attempt $attempt/$max_attempts)${NC}"
        fi
      else
        debug_log "Could not extract block number"
      fi
    else
      debug_log "Could not extract finalized block hash"
    fi
...
debug_log "URL: http://localhost:21000"
...
for i in {1..30}; do
  ...
  if [ $((i % 10)) -eq 0 ]; then
    debug_log "Attempt $i/30 - Response: ${RPC_RESPONSE:0:100}"
  fi
...
if ! echo "$RPC_RESPONSE" | grep -q '"result"'; then
  ...
  echo -e "${RED}Last response: $RPC_RESPONSE${NC}"
  exit 1
fi
